...
Задачі, готові для переносу в трекер. Формат: ID, назва, фаза, ADR, DoD, контрольний лічильник, оцінка. Оцінки в днях для однієї людини.

## Фаза 0

| ID | Задача | DoD | Контрольний лічильник | Оцінка |
| --- | --- | --- | --- | --- |
...
| ID | Задача | ADR | DoD | Контрольний лічильник | Оцінка |
| --- | --- | --- | --- | --- | --- |
...
| T1-09 | API-preparation: no\_dcf\_company |  | s/test 130 → ≤ 45 | machine-time проекту | 3 |
| T1-10 | API-preparation: common\_company |  | s/test 75 → ≤ 40 |  | 5 |
...
| T0-13 | Контрольні лічильники: скрипт, baseline, CI-гейт "тільки вниз" | 0 | `debt-baseline.json` на main дорівнює правді; PR із ростом лічильника червоний; `metrics/debt.jsonl` пишеться на мержі | 1–2 |
| T0-14 | Метрики прогону з JSON-репортера + тижневий звіт | 0 | `metrics/runs.jsonl` з кожного прогону; `metrics/weekly/*.md` щопонеділка; дашборд читає jsonl | 2 |
