...
- [ ] Робочі сесії йдуть на `claude-opus-4-8`: той самий масив у налаштуваннях VS Code задає `ANTHROPIC_MODEL=claude-opus-4-8`. В особистому каталозі його перебиває `"model": "opus[1m]"` у `settings.json`, у робочому такого рядка нема.
...
- `/status`: робоча пошта, Working folder = репо TAF.
