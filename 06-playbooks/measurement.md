...
- На кожному PR: `npm run debt:check`.
  ...
  - будь-яке число в baseline PR > baseline на `origin/main` → fail ("показник може лише зменшуватись").
- На мержі в main: рядок `{date, commit, counters}` → `metrics/debt.jsonl`.
- Атрибуція: `git log -- debt-baseline.json` дає список PR, кожен із яких зменшив конкретні показники.
...
1. Таблиця показників: значення тиждень тому, зараз, дельта, PR, що дали дельту.
